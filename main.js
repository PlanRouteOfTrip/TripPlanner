...
/* harmony import */ var _calculateWithStartFinish__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./calculateWithStartFinish */ "./src/calculateWithStartFinish.js");
/* harmony import */ var _calculateTrips__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./calculateTrips */ "./src/calculateTrips.js");
var infowindow;
var google;
...
document.getElementById("addPoint").addEventListener("click", async function (e) {
  e.preventDefault();
...
document.getElementById("addStart").addEventListener("click", async function (e) {
  e.preventDefault();
  let start = document.getElementById("yourLocation").value;
  startPoint = await getFoundPlace(start); // check if date of trip start is chosen

  startDay = document.getElementById("dateOfTripStart").value;
  console.log("this is start date", startDay); // check if time of trip start is chosen

  startTime = document.getElementById("startTime").value;
  console.log("this is start time", startTime);
}); // pressing of the button to add final point of the trip

document.getElementById("addFinish").addEventListener("click", async function (e) {
  e.preventDefault();
  let end = document.getElementById("finishLocation").value;
  if (!end.length) endPoint = startPoint;else endPoint = await getFoundPlace(end); // check if date of trip end is chosen

  endDay = document.getElementById("dateOfTripEnd").value;
  console.log("this is end date", endDay); // check if time of trip end is chosen
...
document.getElementById("findTrips").addEventListener("click", async function (e) {
  e.preventDefault();
  let withTimeFromStart = await Object(_calculateWithStartFinish__WEBPACK_IMPORTED_MODULE_0__["getTimeFromStart"])(startPoint, points, totalTripTime);
  let withTimeToFinish = await Object(_calculateWithStartFinish__WEBPACK_IMPORTED_MODULE_0__["getTimeToFinish"])(endPoint, withTimeFromStart, totalTripTime);
  ...
  let dt1 = new Date(startDay + "T" + startTime);
  let bestTrips = Object(_calculateTrips__WEBPACK_IMPORTED_MODULE_1__["getSets"])(withTimeToFinish, totalTripTime, dt1);
  ...
  for (let i = 0; i < bestTrips.length; i++) {
    let currentBest = document.createElement("li"); //create string with all the info about current trip option
...
/***/ "./src/calculateTrips.js":
/*!*******************************!*\
  !*** ./src/calculateTrips.js ***!
  \*******************************/
/*! exports provided: getSets */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "getSets", function() { return getSets; });
/* harmony import */ var _openingHours__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./openingHours */ "./src/openingHours.js");
/* harmony import */ var _findThreeBestSets__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./findThreeBestSets */ "./src/findThreeBestSets.js");
/* harmony import */ var _matrixMaker__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./matrixMaker */ "./src/matrixMaker.js");
...
function getSets(places, totalTripTime, start) {
  let finalSet = [];
  let bestSet = [];
  let matrix = Object(_matrixMaker__WEBPACK_IMPORTED_MODULE_2__["fillTravelTimes"])(places);
  console.log('matrix', matrix); // console.log('element in matrix', matrix[0][1])
  ...
  for (let i = 0; i < tripsOptions.length; i++) {
    ...
    for (let j = 1; j < places.length; j++) {
      ...
      time = time + matrix[trip[j - 1].index][trip[j].index] + trip[j].minsToSpend; //if time greater than totalTripTime we need to cut rest of the points in this trip and break this loop
    ...
    trip[0].totalTripTime = time; //check if trip is already in finalSet
  ...
  let afterCheckHours = Object(_openingHours__WEBPACK_IMPORTED_MODULE_0__["checkOpenHours"])(finalSet, start, matrix); // pick 3 best sets

  bestSet = Object(_findThreeBestSets__WEBPACK_IMPORTED_MODULE_1__["getBestSet"])(afterCheckHours);
  return bestSet;
}
  ...
  for (let i = 0; i < curSet.length; i++) {
    if (curSet[i].length === trip.length) {
      ...
      while (idx < trip.length) {
        if (curSet[i][idx].place_id === trip[idx].place_id && curSet[i][idx].minsToSpend === trip[idx].minsToSpend) idx++;else {
          countOfFalse++;
          break;
        }
      }
...
/***/ "./src/calculateWithStartFinish.js":
/*!*****************************************!*\
  !*** ./src/calculateWithStartFinish.js ***!
  \*****************************************/
/*! exports provided: getTimeFromStart, getTimeToFinish */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "getTimeFromStart", function() { return getTimeFromStart; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "getTimeToFinish", function() { return getTimeToFinish; });
//we have
//startPoint = {}
//places = [{}, {}, ...]
//finishPoint = {}
...
async function getTimeFromStart(startPoint, places, totalTripTime = 0) {
    ...
    service.getDistanceMatrix(request, (response, status) => {
      if (status === "OK") {
        ...
        for (let i = 0; i < places.length; i++) {
          ...
          if (curPlace.minsToSpend + curTimeToFinish + curPlace.timeFromStart < totalTripTime) {
...
async function getTimeToFinish(endPoint, places, totalTripTime = 0) {
...
/***/ "./src/findThreeBestSets.js":
/*!**********************************!*\
  !*** ./src/findThreeBestSets.js ***!
  \**********************************/
/*! exports provided: getBestSet */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "getBestSet", function() { return getBestSet; });
...
/*!****************************!*\
  !*** ./src/matrixMaker.js ***!
  \****************************/
/*! exports provided: fillTravelTimes */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "fillTravelTimes", function() { return fillTravelTimes; });
function fillTravelTimes(places) {
  let error = "";
  let travelTimes = [];
  ...
  let addressesOnly = [...places].map(el => el.address); //   console.log("only addresses for matrix", addressesOnly);
...
/*!*****************************!*\
  !*** ./src/openingHours.js ***!
  \*****************************/
/*! exports provided: checkOpenHours */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "checkOpenHours", function() { return checkOpenHours; });
...
//       } else {
//         timer = new Date(
//           new Date(timer).getTime() + (matrix[point.index][places[i - 1].index] * 60 * 1000)
//         );
//       }
...
//       }
//       set.push(point);
//       if (set.length === places.length) availablePlaces.push(set);
//     }
//   }
...
function checkOpenHours(routes, start, matrix) {
  for (let j = 0; j < routes.length; j++) {
    ...
    for (let i = 0; i < places.length; i++) {
      let timeToFirstPoint = places[0].timeFromStart;
      let point = places[i]; // if we on first point need add time from start to know what time we will be there

      if (i === 0) {
        timer = new Date(new Date(start).getTime() + timeToFirstPoint * 60 * 1000); // if not the first point, add time from previous point to current from matrix
      } else {
        timer = new Date(new Date(timer).getTime() + matrix[point.index][places[i - 1].index] * 60 * 1000);
      } // if point has property "hours" need to check if place will be opened
      ...
      if (point.hours) {
        // find day when you will come to the point
        // O - Sunday, 1 - Monday, 2 - Tuesday, 3 - Wednesday, 4 - Thursday, 5 - Friday, 6 - Saturday
        let idxDay = timer.getDay(); // get open and close time of current place
        // 0 - Monday, 1 - Tuesday, 2 - Wednesday, 3 - Thursday, 4 - Friday, 5 - Saturday, 6 - Sunday

        let idxDayOfPlace = idxDay - 1;
        if (idxDayOfPlace < 0) idxDayOfPlace = 6; // check if there is schedule on this day, if not  -> it's closed

        if (point.hours.periods[idxDayOfPlace]) {
          let opening = point.hours.periods[idxDayOfPlace].open.hours;
          let closing = point.hours.periods[idxDayOfPlace].close.hours; // DON'T COUNT MINUTES OF OPENING FOR NOW !!!!!

          let arrivalHours = timer.getHours(); // let arrivalMinutes = timer.getMinutes();
          // check if place is opened at the time of arriving to the point

          if (arrivalHours >= opening && arrivalHours < closing) {
            timer = new Date(new Date(timer).getTime() + point.minsToSpend * 60 * 1000);
            let leavingHours = timer.getHours(); // check if the place is still opened at the time of leaving of the place
